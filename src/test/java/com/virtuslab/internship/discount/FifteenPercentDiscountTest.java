...
class FifteenPercentDiscountTest {
    ...
    @Test
    void shouldApply15PercentDiscountWhenThereAre4GrainProducts() {
        // Given
        var productDb = new ProductDb();
        var bread = productDb.getProduct("Bread");
        var cereals = productDb.getProduct("Cereals");
    ...
    void shouldNotApply15PercentDiscountWhenThereAre2GrainProducts() {
        ...
        var receipt = new Receipt(receiptEntries);
        var discount = new FifteenPercentDiscount();
        var expectedTotalPrice = steak.price().multiply(BigDecimal.valueOf(2)).add(bread.price()).add(cereals.price());
